...
export function registerRoutes(app: Express) {
  ...
  app.patch('/api/user', async (req, res) => {
    try {
      ...
      if (!user) {
        res
          .status(404)
          .json({ message: 'User not found, try refreshing the page' })
        return
      }
  ...
  app.get('/api/drinks', async (req, res) => {
    try {
      ...
      const allDrinks = await db.query.drinks.findMany({
        where: or(eq(drinks.userId, user.id), eq(drinks.isCustom, false)),
      })
      res.json(allDrinks)
    } catch (error) {
  ...
  app.get('/api/intakes', async (req, res) => {
    try {
      ...
      const recentIntakes = await db.query.intakes.findMany({
        where: and(eq(intakes.userId, user.id), gte(intakes.timestamp, dayAgo)),
  ...
  app.delete('/api/intakes/:id', async (req, res) => {
    try {
      const intakeId = Number.parseInt(req.params.id)
      const user = await db.query.users.findFirst()
      ...
      const [deletedIntake] = await db
        .delete(intakes)
        .where(and(eq(intakes.id, intakeId), eq(intakes.userId, user.id)))
        .returning()
